...
switch (modo){
	...
	case SOLID_ILLUMINATED_FLAT:draw_iluminacion_plana( );break; //Pr4
	case SOLID_ILLUMINATED_GOURAUD:draw_iluminacion_suave();break;//Pr4
  case SELECT:draw_seleccion(r1, g1, b1);break; //pr5
	}
}
...
techo.ambiente_difusa=_vertex4f(0.4,0.1,0.9,1.0);  //coeficientes ambiente y difuso
techo.especular=_vertex4f(0.9,0.5,0.0,1.0);        //coeficiente especular
techo.brillo=50;
...
if (activo[0]==1) escalera.draw(modo, r_p, g_p, b_p, r_p, g_p, b_p, grosor);
else escalera.draw(modo, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();

glPushMatrix();
glRotatef(giro_columna,0,1,0);
glPushMatrix();
glTranslatef(0.0,-0.1,0.0);
if (activo[2]==1) columna.draw(modo, r_p, g_p, b_p,r_p, g_p, b_p, grosor);
else columna.draw(modo, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();


glPushMatrix();
glTranslatef(0,0.6,0);
glRotatef(giro_techo,0,0,1);
glRotatef(sube_techo,1,0,0);
if (activo[1]==1) techo.draw(modo, r_p, g_p, b_p, r_p, g_p, b_p, grosor);
else techo.draw(modo, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _triangulos3D::draw_iluminacion_plana( )
{
	...
	glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE);//luz dentro de la figura
	glEnable (GL_LIGHTING);  //Activacion
	...
	glBegin(GL_TRIANGLES);
	for (i=0;i<caras.size();i++){
		glNormal3fv((GLfloat *) &normales_caras[i]); //se pasa la normal  a la cara
	        glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
		glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
		glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
	}
	...
	glDisable(GL_LIGHTING); //se desactiva la iluminacion
}
...
//	glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE);//luz dentro de la figura
	...
	for (i=0;i<caras.size();i++){
		glNormal3fv((GLfloat *) &normales_vertices[caras[i]._0]); //se pasa la normal  a la cara
	  glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
...
void _triangulos3D::calcular_normales_caras()
{
	...
	//calculo, obtenemos dos vectores dentro de cada triangulo
...
void _pantenon::seleccion()
{

	int c;
	c=color_selec[0][0];
	glPushMatrix();
	columna.draw(SELECT, c, c, c, c, c, c, 1);

	c=color_selec[0][1];
	glRotatef(giro_techo,0,1,0);
	glPushMatrix();
	glTranslatef(0.0,(columna.altura+techo.altura)/2.0,0.0);
	techo.draw(SELECT, c, c, c, c, c, c, 1);
	glPopMatrix();

	c=color_selec[0][2];
	glPushMatrix();
	glTranslatef(techo.anchura/2.0,(columna.altura+techo.altura)/2.0,0.0);
	glRotatef(sube_escal,0,0,1);
	escalera.draw(SELECT, c, c, c, c, c, c, 1);
	glPopMatrix();
